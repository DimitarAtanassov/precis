...
class PaperParser:
    ...
    def _get_source(self, path_or_url: str) -> Path | bytes:
    ...
    def _select_parser(self, source: Path | bytes) -> BaseParser:
        ...
        raise ValueError(
            ...
            "Supported formats: PDF with TOC or font-based sections."
        )
    ...
    def _load_all_content(self, paper: ParsedPaper) -> None:
        ...
        def load_recursive(sections: list[Section]) -> None:
            for section in sections:
                if not section.content and self._active_parser is not None:
    ...
    def get_section_content(self, paper: ParsedPaper, section: Section) -> str:
        ...
        if not section.content:
            if self._active_parser is None:
                raise RuntimeError("No active parser set.")
            parser = self._active_parser
            # Use correct method and pass required arguments
            if hasattr(parser, "extract_section_content"):
                content = parser.extract_section_content(
    ...
    def get_toc_preview(self, path_or_url: str) -> list[str]:
        ...
        return ["  " * (level - 1) + title for level, title, _ in toc]
