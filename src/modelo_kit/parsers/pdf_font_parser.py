...
class PDFFontParser(BaseParser):
    ...
    def _detect_body_font_size(self, blocks: list[TextBlock]) -> float:
        ...
        most_common_size = max(size_counts.items(), key=lambda item: item[1])[0]

        return float(most_common_size)

    def _detect_header_sizes(
        self, blocks: list[TextBlock], body_size: float
    ) -> list[float]:
    ...
    def _is_section_header(
        self, block: TextBlock, header_sizes: list[float], body_size: float
    ) -> bool:
    ...
    def _get_header_level(self, font_size: float, header_sizes: list[float]) -> int:
    ...
    def _build_sections(
        self, blocks: list[TextBlock], header_sizes: list[float], body_size: float
    ) -> list[Section]:
